...
import { stageData, setTime } from '../redux/reducers';
...
const cards = [
  ...
  {id : 3, emoji : '🍟', card : 'french-fries', isFlipped : false, isMatch : false},
  ...
  {id : 20, emoji : '🍗', card : 'fried-chicken', isFlipped : false, isMatch : false},
  ...
  {id : 24, emoji : '🥫', card : 'canned-food', isFlipped : false, isMatch : false},
  ...
  {id : 33, emoji : '🍰', card : 'piece-of-cake', isFlipped : false, isMatch : false},
...
function Board() {
  ...
  // 보드의 크기에 따라 달라지는 덱
  const makeCardDeck = useCallback((cards) => {
    let randomNumberArr = [];
  ...
  const getOpenCardArr = useCallback(() => {
    return board.deck.map((el, i) => (el.isFlipped && !el.isMatch ? i : -1)).filter(index => index !== -1);
  }, [board.deck]);
  ...
  let moveStage = useCallback(() => {
    if (stage < 5) {
      ...
      const newBoardSize = boardSize + 8;
      const newTime = 60;
  ...
  }, [board.deck, boardSize, stage, isClear, makeCardDeck, isTry, dispatch, time, moveStage]);
      ...
      <section>
        ...
        {isClear && !isTry ? (
          <Hidden $isHidden={isClear}>
            <Success clearTime={clearTime} stage={stage} moveStage={moveStage} />
          </Hidden>
        ) : isTry && !isClear ? (
